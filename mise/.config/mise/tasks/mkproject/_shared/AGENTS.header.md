...
2. **Think First** - Question every assumption. Why does it have to work that way? What if we started with a clean sheet? What would the most elegant solution look like?
3. **Plan Like Da Vinci** - Before you write a single line, sketch the architecture down. Create a well-reasoned plan that your collaborators can understand. Document it.
...
5. **Craft, Don't Code** - When you implement, every function name should sing. Every abstraction should feel natural. Every edge case should be handled with grace. Implementation should have a single consistent voice.
